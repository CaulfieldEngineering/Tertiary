...
namespace audio_plugin {
    ...
    //==============================================================================
    void AudioPluginAudioProcessor::attachParametersToLayout() {
        ...
        param = dynamic_cast<juce::AudioParameterFloat*>(
        ...
        auto boolHelper = [&apvts = this->apvts, &params](auto& param,
                                                        const auto& paramName) {
        param = dynamic_cast<juce::AudioParameterBool*>(
            apvts.getParameter(params.at(paramName)));
        jassert(param != nullptr);
        };
    ...
    //==============================================================================
    void AudioPluginAudioProcessor::addParameterListeners() {
        /* Local reference to parameter mapping */
        using namespace Params;
        const auto& params = GetParams();
    ...
    juce::AudioProcessorValueTreeState::ParameterLayout AudioPluginAudioProcessor::createParameterLayout() {
        ...
        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Input_Gain), 1},  // Parameter ID
        ...
        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Low_Mid_Crossover_Freq), 1},  // Parameter ID
            params.at(Names::Low_Mid_Crossover_Freq),      // Parameter Name
            NormalisableRange<float>(20, 20000, 1, 0.3f),  // Range
            500));                                         // Default Value

        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Mid_High_Crossover_Freq),
                        1},                                // Parameter ID
        ...
        /* LFO Waveshape */
        // ===================================================================================================================
        sa = {"Ramp Down", "Ramp Up", "Square", "Triangle",
            "Sine",      "Hills",   "Valleys"};

        layout.add(std::make_unique<AudioParameterChoice>(
            ParameterID{params.at(Names::Wave_Low_LFO), 1},  // Parameter ID
            params.at(Names::Wave_Low_LFO),                  // Parameter Name
            sa,                                              // Range
            4));                                             // INDEX of Default Value

        layout.add(std::make_unique<AudioParameterChoice>(
            ParameterID{params.at(Names::Wave_Mid_LFO), 1},  // Parameter ID
            params.at(Names::Wave_Mid_LFO),                  // Parameter Name
            sa,                                              // Range
            4));                                             // INDEX of Default Value

        layout.add(std::make_unique<AudioParameterChoice>(
            ParameterID{params.at(Names::Wave_High_LFO), 1},  // Parameter ID
        ...
        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Depth_Low_LFO), 1},  // Parameter ID
            params.at(Names::Depth_Low_LFO),                  // Parameter Name
            depthRange,                                       // Range
            75));                                             // Default Value

        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Depth_Mid_LFO), 1},  // Parameter ID
            params.at(Names::Depth_Mid_LFO),                  // Parameter Name
            depthRange,                                       // Range
            75));                                             // Default Value

        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Depth_High_LFO), 1},  // Parameter ID
        ...
        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Invert_Low_LFO), 1},  // Parameter ID & Hint
            params.at(Names::Invert_Low_LFO),                  // Parameter Name
            false));                                           // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Invert_Mid_LFO), 1},  // Parameter ID & Hint
            params.at(Names::Invert_Mid_LFO),                  // Parameter Name
            false));                                           // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Invert_High_LFO), 1},  // Parameter ID & Hint
        ...
        // ===================================================================================================================
        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Sync_Low_LFO), 1},  // Parameter ID & Hint
            params.at(Names::Sync_Low_LFO),                  // Parameter Name
            true));                                          // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Sync_Mid_LFO), 1},  // Parameter ID & Hint
            params.at(Names::Sync_Mid_LFO),                  // Parameter Name
            true));                                          // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Sync_High_LFO), 1},  // Parameter ID & Hint
        ...
        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Rate_Low_LFO), 1},  // Parameter ID & Hint
            params.at(Names::Rate_Low_LFO),                  // Parameter Name
            rateRange,                                       // Range
            3.f));                                           // Default Value

        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Rate_Mid_LFO), 1},  // Parameter ID & Hint
            params.at(Names::Rate_Mid_LFO),                  // Parameter Name
            rateRange,                                       // Range
            3.f));                                           // Default Value

        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Rate_High_LFO), 1},  // Parameter ID & Hint
        ...
        sa = {"1/2 Note", "1/4 Note",         "1/4 Note Triplet",
            "1/8 Note", "1/8 Note Triplet", "1/16 Note"};
        ...
        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Relative_Phase_Low_LFO),
                        1},                            // Parameter ID & Hint
        ...
        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Relative_Phase_Mid_LFO),
                        1},                            // Parameter ID & Hint
        ...
        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Relative_Phase_High_LFO),
                        1},                             // Parameter ID & Hint
        ...
        // ===================================================================================================================
        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Bypass_Low_Band), 1},  // Parameter ID & Hint
            params.at(Names::Bypass_Low_Band),                  // Parameter Name
            false));                                            // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Bypass_Mid_Band), 1},  // Parameter ID & Hint
            params.at(Names::Bypass_Mid_Band),                  // Parameter Name
            false));                                            // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Bypass_High_Band),
                        1},                      // Parameter ID & Hint
        ...
        // ===================================================================================================================
        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Mute_Low_Band), 1},  // Parameter ID & Hint
            params.at(Names::Mute_Low_Band),                  // Parameter Name
            false));                                          // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Mute_Mid_Band), 1},  // Parameter ID & Hint
            params.at(Names::Mute_Mid_Band),                  // Parameter Name
            false));                                          // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Mute_High_Band), 1},  // Parameter ID & Hint
        ...
        // ===================================================================================================================
        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Solo_Low_Band), 1},  // Parameter ID & Hint
            params.at(Names::Solo_Low_Band),                  // Parameter Name
            false));                                          // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Solo_Mid_Band), 1},  // Parameter ID & Hint
            params.at(Names::Solo_Mid_Band),                  // Parameter Name
            false));                                          // Default Value

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Solo_High_Band), 1},  // Parameter ID & Hint
        ...
        // ===================================================================================================================
        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Gain_Low_Band), 1},  // Parameter ID & Hint
            params.at(Names::Gain_Low_Band),                  // Parameter Name
            gainRange,                                        // Range
            0));                                              // Default Value

        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Gain_Mid_Band), 1},  // Parameter ID & Hint
            params.at(Names::Gain_Mid_Band),                  // Parameter Name
            gainRange,                                        // Range
            0));                                              // Default Value

        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Gain_High_Band), 1},  // Parameter ID & Hint
            params.at(Names::Gain_High_Band),                  // Parameter Name
            gainRange,                                         // Range
            0));                                               // Default Value

        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Output_Gain), 1},  // Parameter ID & Hint
        ...
        // ===================================================================================================================
        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Show_Low_Scope), 1},  // Parameter ID & Hint
            params.at(Names::Show_Low_Scope),                  // Parameter Name
            true));

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Show_Mid_Scope), 1},  // Parameter ID & Hint
            params.at(Names::Show_Mid_Scope),                  // Parameter Name
            true));

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Show_High_Scope), 1},  // Parameter ID & Hint
            params.at(Names::Show_High_Scope),                  // Parameter Name
            true));

        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Stack_Bands_Scope),
                        1},                       // Parameter ID & Hint
        ...
        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Scope_Point1), 1},  // Parameter ID & Hint
            params.at(Names::Scope_Point1),                  // Parameter Name
            pointRange,                                      // Range
            25.f));                                          // Default Value

        layout.add(std::make_unique<AudioParameterFloat>(
            ParameterID{params.at(Names::Scope_Point2), 1},  // Parameter ID & Hint
        ...
        // ===================================================================================================================
        layout.add(std::make_unique<AudioParameterBool>(
            ParameterID{params.at(Names::Show_FFT), 1},  // Parameter ID & Hint
        ...
        layout.add(std::make_unique<AudioParameterChoice>(
            ParameterID{params.at(Names::FFT_Pickoff), 1},  // Parameter ID & Hint
    ...
    /* Called upon any change in multiplier value.  Forces all LFO read-back
       positions to be synchronized with each other (when no host BPM), or forces
       all LFO read-back positions to be synchronized with the host grid */
    //==============================================================================
    void AudioPluginAudioProcessor::forceSynchronization(LFO& lfo) {
      if (/*hostInfo.isPlaying && */ lfo.isSyncedToHost()) {
        ...
        float newPhase =
            playPositionScaled * lfo.getWaveMultiplier() * lfo.getWaveTableSize();
    ...
    //==============================================================================
    void AudioPluginAudioProcessor::setFftPickoffPoint(int point) {
      switch (point) {
        case 0: {
          fftPickoffPointIsInput = false;
          break;
        }

        case 1: {
          fftPickoffPointIsInput = true;
          break;
        }
      }
    }
    ...
    //==============================================================================
    float AudioPluginAudioProcessor::getRmsValue(const int pickOffPoint,
    ...
    //==============================================================================
    void AudioPluginAudioProcessor::updateState() {
      ...
      /* Sometimes when multiplier param is changed, LFOs become out of sync with
         each other or out-of-sync with the host grid.  The following detects
         Multiplier changes, and forces the synchronization. Cannot remember why,
         but didn't seem to work when placed in parameterChanged callback */
      ...
      /* Furthermore, we reset the LFO position on each play-after-pause to force
         the synchronization of the LFO's. An SR-Latch based on the user's hit of
         Play and Pause is what triggers the force synchronization */
    ...
    //==============================================================================
    void AudioPluginAudioProcessor::pushSignalToFFT(
    ...
    /* Pulls RMS audio levels from ProcessBlock and updates the Input and Output
     * meters. */
    //==============================================================================
    void AudioPluginAudioProcessor::updateMeterLevels(
        juce::LinearSmoothedValue<float>& left,
      ...
      const auto newValueLeft = juce::Decibels::gainToDecibels(
      ...
      if (newValueLeft < left.getCurrentValue())
        ...
        left.setCurrentAndTargetValue(newValueLeft);

      /* If Stereo, do the same to the right */
      if (getTotalNumInputChannels() > 1) {
        right.skip(totalNumSamples);

        const auto newValueRight = juce::Decibels::gainToDecibels(
        ...
        if (newValueRight < right.getCurrentValue())
          ...
          right.setCurrentAndTargetValue(newValueRight);
      } else
        right = left;
    }
    ...
    //==============================================================================
    void AudioPluginAudioProcessor::applyCrossover(
      ...
      auto fb0Block = juce::dsp::AudioBlock<float>(
          filterBuffers[0]);  // Create audio block for the buffer containing LOWS
      auto fb1Block = juce::dsp::AudioBlock<float>(
          filterBuffers[1]);  // Create audio block for the buffer containing MIDS
      auto fb2Block = juce::dsp::AudioBlock<float>(
          filterBuffers[2]);  // Create audio block for the buffer containing HIGHS

      auto fb0Ctx = juce::dsp::ProcessContextReplacing<float>(
          fb0Block);  // Create a context for the block
      auto fb1Ctx = juce::dsp::ProcessContextReplacing<float>(
          fb1Block);  // Create a context for the block
      auto fb2Ctx = juce::dsp::ProcessContextReplacing<float>(
          fb2Block);  // Create a context for the block
      ...
      filterBuffers[2] =
          filterBuffers[1];  // Make a copy before BPF, to prepare for HIGHS
    ...
    //==============================================================================
    void AudioPluginAudioProcessor::applyAmplitudeModulation() {
      /* Apply Amplitude Modulation */
      for (int sample = 0; sample < filterBuffers[0].getNumSamples(); ++sample) {
        ...
        int lowIndex =
            fmod((lowLFO.getPosition() + lowLFO.getRelativePhaseInSamples()),
                 (float)waveTableSize);
        int midIndex =
            fmod((midLFO.getPosition() + midLFO.getRelativePhaseInSamples()),
                 (float)waveTableSize);
        int highIndex =
            fmod((highLFO.getPosition() + highLFO.getRelativePhaseInSamples()),
                 (float)waveTableSize);
        ...
        lowLFO.setPosition(
            fmod((lowLFO.getPosition() + lowLFO.getIncrement()), waveTableSize));
        midLFO.setPosition(
            fmod((midLFO.getPosition() + midLFO.getIncrement()), waveTableSize));
        highLFO.setPosition(
    ...
    //==============================================================================
    void AudioPluginAudioProcessor::sumAudioBands(
        juce::AudioBuffer<float>& buffer) {
      using namespace juce;
      ...
      auto addFilterBand = [nc = numChannels, ns = numSamples](auto& inputBuffer,
                                                               const auto& source) {
        for (auto i = 0; i < nc; ++i) {
